...
<%
	...
	Cookie[] cookies = request.getCookies(); 
 	for(int i = 0; cookies != null && i < cookies.length; i++){
		...
		if(cookie.getName().equals("managerInfo")){
			...
			if (quit != null){
				cookie = new Cookie("managerInfo", userType + "," + uid + ","+ name);
				cookie.setMaxAge(0);
 	...
 	if(userInfo == null || userInfo.trim().equals("")){ 
 		...
 		if (userType == null || userType.trim().equals("")){
 			%>
    		<jsp:forward page="/login/manager_login.jsp">
	...
	if (loggedin == false){
		...
		cookie.setMaxAge(Integer.MAX_VALUE); 
 		response.addCookie(cookie); 	
		if(userType.equals("office")){
			%>
			<jsp:forward page="/manage/office.jsp">
			...
			%>
			<jsp:forward page="/manage/admin.jsp">
				<jsp:param name="userid" value="<%=uid%>" />
				<jsp:param name="username" value="<%=name%>" />
		    </jsp:forward>
	    	<%
